...
export class RegisterDoctorComponent implements OnInit {
  ...
  constructor(private userService: UserService, 
    private servic_manager: MenagerService,
  ...
  ngOnInit(): void {
    this.servic_manager.get_all_spec().subscribe((data) => {
      let i = 0;
  ...
  async registerUser() {
    ...
    if (flag) {
      ...
      let number_licenc = $("#num_doc_lic").val() as string;
      let office_branch = $("#office_branch").val() as string;
      let specializzation = $(".select-btn #s1").text() as string;
      ...
      if (error_message.length != 0) {
        ...
        const data_package = {
          ...
          password: password,
          street: street,
          number_licenc: number_licenc,
          office_branch: office_branch,
          specializzation: specializzation,
          img_src: data,
          extension: this.file_extension
        };
        this.userService.register_doctor(data_package).subscribe(async (response) => {
