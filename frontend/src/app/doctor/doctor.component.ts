...
export class DoctorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
      ...
      if (data['status'] === 200) {
        ...
        this.servic.getSpecServicData(token).subscribe((data) => {
          if (data['status'] === 200) {
            ...
            this.servic
              .get_expiration_time(sessionStorage.getItem('user_token'))
              .subscribe((response) => {
                let time =
                  response['data'].houers * 60 * 60 * 1000 +
                  response['data'].minuttes * 60 * 1000 +
            ...
            this.servic
              .get_calender_for_doctor(sessionStorage.getItem('user_token'))
              .subscribe((data) => {
                if (data['status'] == 200) {
                 ...
                 this.last_3 = this.calander_data.filter(item => !item.cancled && item.status=='ongoing')
                  .filter(item => {
                    // const ds = new Date(item.date_start)
                    // const de = new Date(item.date_end)
                    // let sugger0 = (ds.getMonth()+1)+"."+ds.getDate();
                    // let sugger1 = ds.getHours() +":"+ds.getMinutes();
                    // let sugger2 = de.getHours() +":"+de.getMinutes();
                    // return{ ...item, 
                    //   timeDifference: Math.abs(ds.getTime() - currentTime.getTime()),
                    //   sugger0: sugger0,
                    //   sugger1: sugger1,
                    //   sugger2: sugger2
                    // }
                    const curr_date =  new Date();
                    const my_ap = new Date(item.date_start);
                    if(my_ap.getDate() >= curr_date.getDate()){
                      return true
                    }else{
                      return false
                    }
                  })
                  .sort((a, b) => new Date(a.date_start).getDate() - new Date(b.date_start).getDate())
                  .map(item=>{
                      const ds = new Date(item.date_start)
                    const de = new Date(item.date_end)
                    let sugger0 = (ds.getMonth()+1)+"."+ds.getDate();
    ...
    $(document).ready(() => {
      ...
      $('#date_submit').on('click', () => {
        ...
        if (
          new Date(this.sdate) <= currentdate ||
          new Date(this.edate) <= currentdate
        ) {
        ...
        this.servic
          ...
          .subscribe((data) => {
            if (data['status'] == 200) {
              ...
              $('.top-message span').text("Sorry you can't submit...");
              $('.bottom-message span').text(
                'Dates are overlaping with existing datees!'
              );
              $('.bottom-next span').text('Return to the form');
              $('.popup-background').addClass('popup-active');
              $('#breakDateend').addClass('input-box-empty');
  ...
  openForm(id) {
    ...
    elementsForTargetDate.forEach((elemnt) => {
      ...
      if (elemnt.cancled === true) {
        ...
        this.renderer.appendChild(cal_data, s);
      } else {
        this.renderer.appendChild(
          cal_data,
          this.renderer.createText(`${firstname} ${lasttane}:  ${start}-${end}`)
      ...
      if (isPast && elemnt.cancled == false) {
      ...
      if (
        ...
        elemnt.doctorts_note == false &&
        !this.isDateInPast(dann, mesec, godina)
      ) {
      ...
      this.renderer.listen(cal_data, 'click', () => {
        this.servic
          .get_user_specific_reports(elemnt.patient_id)
          .subscribe((data) => {
            if (data['status'] === 200) {
              this.user_specific_reports = data['data'].map(
                (elem: ReportData) => {
      ...
      this.renderer.listen(noteImg, 'click', () => {
        ...
        this.createReport(
          this.el.nativeElement.querySelector(`#${noteImgClass}`)
        );

        this.el.nativeElement.querySelector(`#${noteImgClass}`).style.display =
          'block';
        cancelImg.style.display = 'none';
        noteImg.style.display = 'none';
      ...
      this.renderer.listen(xImg, 'click', () => {
        ...
        this.el.nativeElement.querySelector(`#${id_b_d}`).style.display =
          'none';
        ...
        chechlImg.style.display = 'none';
        chechlImg2.style.display = 'none';
        // this.el.nativeElement.querySelector(`#xmark`).style.display =
        // 'none';
      ...
      this.renderer.listen(chechlImg, 'click', () => {
        try {
          this.report_save(elemnt, id_b_d, noteImg);
        } catch (err) {
          cancelImg.style.display = 'none';
          noteImg.style.display = 'block';
          chechlImg.style.display = 'none';
        ...
        this.el.nativeElement.querySelector(`#${id_b_d}`).style.display =
          'none';
        this.destrory_parent('report_div');
        //const isPast = this.isDateInPast(dann, mesec, godina);
        cancelImg.style.display = 'none';
        noteImg.style.display = 'block';
        chechlImg.style.display = 'none';
      ...
      this.renderer.listen(chechlImg2, 'click', () => {
        ...
        this.servic.cancle_op_as_doc(data).subscribe((data) => {
          if (data['status'] === 200) {
            ...
            let index = this.calander_data.findIndex(
              (elem2) =>
                new Date(elem2.date_start) === new Date(retVal.date_start) &&
                new Date(elem2.date_end) === new Date(retVal.date_end)
            );

            if (index >= 0) {
              this.calander_data[index].cancled = retVal;
            }
  ...
  report_save(elemnt: CalanderData, id, noteImg) {
    const sessio_date = this.el.nativeElement.querySelector(
      `#${id} .session_date`
    ).value;
    this.reportData.next_session = sessio_date;

    const reason_data = this.el.nativeElement.querySelector(
      `#${id} .reas_com_text`
    ).value;
    this.reportData.reason_for_comming = reason_data;

    const diagno_text_date = this.el.nativeElement.querySelector(
      `#${id} .diagno_text`
    ).value;
    this.reportData.diagnosis = diagno_text_date;

    const therapy_text_data = this.el.nativeElement.querySelector(
      `#${id} .therapy_text`
    ).value;
    ...
    if (
      reason_data.trim() === '' ||
      diagno_text_date.trim() === '' ||
      therapy_text_data.trim() === ''
    ) {
    ...
    this.servic.generate_report(elemnt,this.reportData).subscribe((data) => {
      if (data['status'] === 200) {
        console.log('poslato');
        elemnt.doctorts_note = true;
        let index = this.calander_data.findIndex(elem=>new Date(elem.date_start).getDate() === new Date(elemnt.date_start).getDate())
        if(index){
          this.calander_data[index].doctorts_note = true;
        }
        noteImg.style.display = 'none';
        this.cdr.detectChanges();
      } else {
  ...
  new_req: NewServicRequest = new NewServicRequest();

  submit_servic_request() {
    const timeRegex = /^(?:[01]\d|2[0-3]):[0-5]\d$/;
    if (
      this.new_req.service_name === '' ||
      this.new_req.servic_cost == null ||
      !timeRegex.test(this.new_req.service_dur) ||
      this.new_req.service_des.trim() === ''
    ) {
      $('.popup-top').removeClass('success');
      $('.top-image img').attr(
        'src',
      ...
      this.servic
        .set_new_req_spec(this.new_req, this.doctor.d_data['specializzazione'])
        .subscribe((data) => {
          if (data['status'] == 200) {
            $('.popup-top').addClass('success');
            $('.top-image img').attr(
          ...
          this.new_req = new NewServicRequest();
          this.cdr.detectChanges();
        });
    }
  }
  ...
  isDateInPast(dd, mm, yyyy) {
   ...
   date_inspection(element: CalanderData) {
    ...
    if (flag) {
      this.servic
        .get_doctor_services_update_date(element, this.doctor._id.toString())
        .subscribe((data) => {
          if (data['status'] === 200) {
          } else {
            console.log(data['error_message']);
